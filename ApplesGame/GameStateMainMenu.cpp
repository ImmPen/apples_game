...
namespace ApplesGame
{
	...
	{
		data.font.loadFromFile(RESOURCES_PATH + "arial.ttf");
		...
		data.menu.rootItem.children.push_back(&data.recordTableItem);
		...
		data.recordTableItem.text.setString("Record Table");
		data.recordTableItem.text.setFont(data.font);
		data.recordTableItem.text.setCharacterSize(24);
		data.recordTableItem.hintText.setString("Record Table");
		data.recordTableItem.hintText.setFont(data.font);
		data.recordTableItem.hintText.setCharacterSize(48);
		data.recordTableItem.hintText.setFillColor(sf::Color::Red);
		data.recordTableItem.children.push_back(&data.tableItem);

		data.tableItem.text.setString("Records: ");
		for (const RecordsTableItem& item : game.recordsTable.entry)
		{
			data.tableItem.text.setString(data.tableItem.text.getString() + "\n" + item.name + ": " + std::to_string(item.score));
		}
		data.tableItem.text.setFont(data.font);
		data.tableItem.text.setCharacterSize(24);
	...
	void HandleGameStateMainMenuWindowEvent(GameStateMainMenuData& data, Game& game, const sf::Event& event)
	{
		...
		if (event.type == sf::Event::KeyPressed)
		{
			...
			else if (event.key.code == sf::Keyboard::Enter)
			{
				...
				{
					game.options = (GameOptions)((std::uint8_t)game.options ^ (std::uint8_t)GameOptions::ENDLESS_MODE);
				}
				else if (data.menu.selectedItem == &data.optionsWithAccelerationItem)
				{
					game.options = (GameOptions)((std::uint8_t)game.options ^ (std::uint8_t)GameOptions::ACCELERATION_MODE);
				}
				else if (data.menu.selectedItem == &data.recordTableItem)
				{
					ExpandSelectedItem(data.menu);
				}
	...
	{
		bool isInfiniteApples = ((std::uint8_t)game.options & (std::uint8_t)GameOptions::ENDLESS_MODE) != (std::uint8_t)GameOptions::Empty;
		data.optionsInfiniteApplesItem.text.setString("Infinite Apples: " + std::string(isInfiniteApples ? "On" : "Off"));

		bool isWithAcceleration = ((std::uint8_t)game.options & (std::uint8_t)GameOptions::ACCELERATION_MODE) != (std::uint8_t)GameOptions::Empty;
